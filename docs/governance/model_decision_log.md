...
| MD-13 | Platt and isotonic calibration materially improve late-July calibration, but both worsen calibration in early July; Platt has the best aggregate calibration while preserving ranking exactly. | Do not yet freeze a universal post-hoc calibrator. Keep Platt scaling as the leading calibration candidate and investigate why calibration direction changes across periods. | Calibration quality is regime-sensitive. A method that corrects late-July overprediction can worsen earlier underprediction, so aggregate averages are not enough for a production decision. | Active. Next step: test whether recalibration should depend on recent-window prevalence / temporal regime and assess rolling calibration stability. |
